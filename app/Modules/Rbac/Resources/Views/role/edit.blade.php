<div class="easyui-layout validform" data-options="fit:true,border:false" style="width: 450px; height:500px; margin: 10px;">
    <div data-options="region:'north',border:true" style="border:1px solid #ccc;height:200px;pediting: 10px;">
        <form id="jq_edit_form_role" action="/rbac/role/edit" method="post">
        {!! csrf_field() !!}
        ...
        <table style="table-layout:fixed;">
            <tr>
                ...
                <td>
                    <select id="pid" name="pid" class="inputxt easyui-combobox" data-options="panelHeight:140,height:32,width:202,onSelect: function(rec){$('#pid').val(rec.value);}">
                        ...
                        <option value="{{ $item['id'] }}" @if($item['id'] == $role->parent_id) selected @endif>{{ $item['html'].$item['name'] }}</option>
                        @endforeach
                    </select>
                </td>
            ...
            <tr>
                ...
                <td>
                    <input type="text" id="name" name="name" value="{{ $role->name }}" class="inputxt" nullmsg="请输入角色名称" datatype="*2-24" />
                </td>
                <td class="need">*</td>
            </tr>
            <tr>
                ...
                <td>
                    <input type="text" id="slug" name="slug" value="{{ $role->slug }}" class="inputxt" nullmsg="请输入角色别名" datatype="*2-24" />
                </td>
                <td class="need">*</td>
            </tr>
            <tr>
                ...
                <td colspan="3">
                    <textarea name="description" style="width:300px; height: 30px;" datatype="*1-128" ignore="ignore">{{ $role->description }}</textarea>
                </td>
            </tr>
        </table>
    </form>
    </div>
    <div data-options="region:'center',border:false,fit:false" style="padding-top: 10px; padding-bottom: 5px;">
        <input type="checkbox" id="jq_all_privilege" value=""/>&nbsp;全选角色权限
    </div>
    <div data-options="region:'south',border:false" style="border:1px solid #ccc;height:280px;padding: 5px; overflow:scroll;">
...
require(['util', 'easyui', 'valid_form', 'valid_form_datatype'], function(util) {
    ...
    util.createWindow['jq_edit_form_role'] = $('#jq_edit_form_role').Validform(formOptions);
    $('#jq_edit_form_role input').focus(function() {
    ...
    $('#ja_privilege_tree').tree({
        ...
        onlyLeafCheck: true,
        url : '/rbac/role/menu-permission-tree.json?rid={{ $role->id }}',
