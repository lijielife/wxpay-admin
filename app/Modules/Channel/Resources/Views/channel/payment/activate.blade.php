<div class="validform business_info" style="padding: 0 10px;">
    ...
    <table style="table-layout:fixed; width: 100%;" cellspacing="10">
        ...
        <tr>
            ...
            <td>
                <input type="text" id="billing_rate" name="billing_rate" value="{{ $channelPayment->billing_rate }}" class="inputxt" datatype="isDecimals2" nullmsg="请输入结算费率" errormsg="结算费率只能是数字，且小数点最多两位"/>
            </td>
            <td class="need">*</td>
            <td><div class="Validform_checktip"></div></td>
        </tr>
    ...
    <form id="jq_edit_form_channelPayConf" action="/channel/payment/save" method="post">
    {!! csrf_field() !!}
    <input type="hidden" name="id" value="{{ $channelPayment->id }}"/>
    <input type="hidden" name="channel_id" value="{{ $channelPayment->channel_id }}"/>
    <input type="hidden" id="jq_edit_form_channelPayConf_activateStatus" name="activate_status"/>
...
require(['util', 'easyui.widget.extend'], function(util) {
    util.createWindow['jq_edit_form_channelPayConf'] = $('#jq_edit_form_channelPayConf').Validform({confirmBeforeSubmit: true});

    $('#jq_edit_form_channelPayConf input').focus(function() {
